 ...
 * If we can use 62 characters [A-Z, a-z, 0-9] for the short URLs having length n, then we can have total 62^n URLs.
 ...
 * When a user inputs a long URL “http://www.google.com”, the system creates a random 7-character string like “abcd123” as ID and inserts entry <“abcd123”, “http://www.google.com”> into the database.
 ...
 * We can't generate unique hash values for the given long URL.
 * In hashing, there may be collisions (2 long urls map to same short url) and we need a unique short url for every long url so that we can access long url back but hash is one way function.
 *
 * Better Solution:
 ...
 * The idea is simple, get a hash code from original URL and go to corresponding machine then use the same process as a single machine.
 * For routing to the correct node in cluster, Consistent Hashing is commonly used.
 *
 * Following is the pseudocode for example,
 *
 * Get shortened URL
 ...
 * If you dig into the insert/query process, you will notice that using random string as IDs may sacrifice performance a little bit.
 * More specifically, when you already have millions of records, insertion can be costly.
 * Since IDs are not sequential, so every time a new record is inserted, the database needs to go look at the correct page for this ID. However, when using incremental IDs, insertion can be much easier – just go to the last page.
 */